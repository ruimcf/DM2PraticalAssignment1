...
author: "Pedro Belem 201602876, Rui Fonseca 201602879"
date: "18 March 2017"
output:
...
### Serious Accidents
The goal was to find rules that cause serious accidents.
```{r, results="hide"}
rules <- (sort(subset(apriori(data.discretized, param = list(minlen = 1, support = 0.1, confidence = 0.1)), subset = rhs %in% "Accident_Severity=Serious"), by="confidence"))
```
```{r}
inspect(rules)
```

As there are no rules with high confidence, we decided to lower the minimum support to 0.01, since there are a lot less serious accidents.
```{r, results="hide"}
rules <- (sort(subset(apriori(data.discretized, param = list(minlen = 1, support = 0.01, confidence = 0.26)), subset = rhs %in% "Accident_Severity=Serious"), by="confidence"))
```
```{r}
inspect(rules)
```

However, we still couldn't find any rules with high confidence. Still:
...
As an attempt to retrieve more meaningful information, we decided to do some changes. The aditional time and space data was removed (Day of week, day, month, hour, police force) and the rule length was limited to 2.
```{r, echo=FALSE}
...
We can see that increasing the number of casualities is correlated with higher confidence. This relationship makes sense and is expected, as accidents envolving a lot of casualties need a serious motive to happen.

However, we still didn't find rules with high confidence.